...
def filter_main(fastq1, fastq2, chromap_index,chromap_index_fa,mapq, outdir, prefix, threads, optical_duplicate_distance, to_file = False):
	sys.stdout = logger.Logger(outdir + "/" + prefix + ".feather.log")
	print(time.ctime() + " starting mapping and filtering operation")
	check_arguments(fastq1, fastq2, chromap_index, mapq, threads)
	paired_filename, chromap_filename, bwa2_filename, chromap_sorted_filename, bwa2_sorted_filename, combined_chromap_filename, qc_filename = set_filenames(fastq1, fastq2, outdir, prefix)
	#running bwa mem
	# for fastq, sam_filename in [(fastq1, chromap_filename), (fastq2, bwa2_filename)]:
		# if fastq.endswith(".fastq") or fastq.endswith("fastq.gz") or fastq.endswith("fq") or fastq.endswith("fq.gz"):
		# 	chromap_cmd(fastq1,fastq2, chromap_index, threads, sam_filename)
	...
	if fastq1.endswith(".fastq") or fastq1.endswith("fastq.gz") or fastq1.endswith("fq") or fastq1.endswith("fq.gz"):
			chromap_cmd(fastq1,fastq2, chromap_index,chromap_index_fa,threads, chromap_filename)
	elif not (fastq1.endswith(".sam") or fastq1.endswith(".bam")):
			exit("Error: Input file for filtering should be of type fastq, fastq.gz, sam, or bam. Exiting!")
			
	if chromap_filename.endswith(".bam"):
		proc = subprocess.Popen("samtools view " + chromap_filename + " | awk ' $1 !~ /@/ {print $1}' " + "| uniq -c|wc -l", stdout = subprocess.PIPE, shell = True)
		read_count = proc.stdout.read().decode("utf-8")
	else:
		proc = subprocess.Popen("awk ' $1 !~ /@/ {print $1}' " + chromap_filename  + "| uniq -c|wc -l", stdout = subprocess.PIPE, shell = True)
		read_count = proc.stdout.read().decode("utf-8")

	#pairing and filtering alignments for chimeric reads
	chromap = pysam.AlignmentFile(chromap_filename)
	if not is_sorted_queryname(chromap.header):
			print(time.ctime() + " calling samtools sort for " + chromap_filename + " storing in " + chromap_sorted_filename)
			pysam.sort("-o", chromap_sorted_filename , "-n", "-@", str(threads), chromap_filename)
	else:
	  copyfile(chromap_filename, chromap_sorted_filename)
	
	# print(time.ctime() + " merging " + chromap_sorted_filename + " and " + bwa2_sorted_filename)
	# pysam.merge("-n", "-f",  combined_chromap_filename, chromap_sorted_filename, bwa2_sorted_filename)
	
	print(time.ctime() + " merging " + chromap_sorted_filename)
	pysam.merge("-n", "-f",  combined_chromap_filename, chromap_sorted_filename)
	# 
	# print(time.ctime() + " filtering and pairing reads")
	# # filter_pair_reads(combined_chromap_filename, mapq, paired_filename, qc_filename)
	# 
	# 
	...
	try:
		...
		# Fallback to samtools-1.6 compatible version
	...
	##Temporarily, could be used again
	#print(time.ctime() + " calling samtools flagstat on mapped and duplicate-removed file")
	#proc = subprocess.Popen("samtools flagstat " + paired_filename + ".rmdup.bam > " + paired_filename + ".rmdup.flagstat", shell = True)
	#proc.communicate()
	...
	# #pysam.sort("-n", "-o", sam_filename + ".srtn.rmdup.bam", paired_filename + ".rmdup.bam")
...
def filter_pair_reads(sam_filename, mapq, paired_filename, qc_filename):
	merged_sam = pysam.AlignmentFile(sam_filename, "rb")
...
def set_filenames(fastq1, fastq2, outdir, prefix):
	...
	if fastq1.endswith(".fastq") or fastq1.endswith(".fastq.gz") or fastq1.endswith(".fq") or fastq1.endswith(".fq.gz"):
		...
		chromap_filename = tempdir + "/" + fastq1_prefix + ".sam"
		chromap_sorted_filename = chromap_filename + ".srtn"
	elif fastq1.endswith(".sam") or fastq1.endswith(".bam"):
		setname = fastq1[ (fastq1.rfind("/") + 1) : ]
		chromap_filename = fastq1
		chromap_sorted_filename = tempdir + "/" + setname + ".srtn"
	...
	return(paired_filename, chromap_filename, bwa2_filename, chromap_sorted_filename, bwa2_sorted_filename, combined_chromap_filename, qc_filename)
...
def chromap_cmd(fastq1,fastq2, chromap_index,chromap_index_fa, threads, output_filename):
	...
	print("chromap","--preset","hic","-t",str(threads), "-x", chromap_index,"-r" , chromap_index_fa, "-1" ,fastq1,"-2" ,fastq2,"--SAM", "-o",output_filename)
	#proc = subprocess.Popen(["/home/mishras10/chromap/chromap", "-x", "/home/mishras10/mm10.index","-r" , chromap_index, "-1" ,fastq,"--SAM", "-o",output_filename], stderr = open(output_filename + ".log", 'w'))
	proc = subprocess.Popen(["chromap","--preset","hic","-t",str(threads), "-x", chromap_index,"-r" , chromap_index_fa, "-1" ,fastq1,"-2" ,fastq2,"--SAM", "-o",output_filename], stderr = open(output_filename + ".log", 'w'))
	proc.wait()
	#output_file.close()
...
def check_arguments(fastq1, fastq2, chromap_index, mapq, threads):
