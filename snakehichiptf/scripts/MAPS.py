...
def validate_input_data(input_data):
    ...
    if 'MACS2_PATH' in input_data:
        params['MACS2_PATH'] = str(input_data['MACS2_PATH'][1])
        params['USE_MACS2'] = os.path.exists(params['MACS2_PATH'])
    else:
        params['USE_MACS2'] = False
...
def load_metadata(GF_PATH, BIN_SIZE):
    ...
    for col in ['start', 'end', 'effective_length', 'gc', 'mappability']:
        metadata_full[col] = pd.to_numeric(metadata_full[col], errors='coerce')
...
def parse_fname(chrom, type, params):
    ...
    for p in params:
        pn = '[' + p + ']'
        if pn in fname:
            fname = fname.replace(pn, str(params[p]))
...
def init(p):
    ...
    if params.get('USE_MACS2', False):
        print('loading MACS2 peaks')
        print(params['MACS2_PATH'])
        MACS2_full = load_MACS2(params['MACS2_PATH'])
    else:
        print('No MACS2 peaks provided or file does not exist, proceeding without peak classification')
        print(params['MACS2_PATH'])
        MACS2_full = None
    ...
    for CHR in chroms:
        ...
        # Check if short.bed exists
        short_fname = parse_fname(CHR, 'short', params)
        if not os.path.exists(short_fname):
            print(f"No short reads file found for chromosome {CHR} ({short_fname}). Skipping chromosome.")
            qc_str += (
                f'AND_set\t0\tnumber of pairs in AND set at chromosome {CHR} (no short reads file)\n'
                f'XOR_set\t0\tnumber of pairs in XOR set at chromosome {CHR} (no short reads file)\n'
                f'NOT_set\t0\tnumber of pairs in NOT set at chromosome {CHR} (no short reads file)\n'
            )
            continue

        print('-- handling short.bed\n')
        ps_short = pd.read_csv(short_fname, header=None, sep='\t')
        if ps_short.shape[0]:
            ...
            long_fname = parse_fname(CHR, 'long', params)
            if not os.path.exists(long_fname):
                print(f"No long reads file found for chromosome {CHR} ({long_fname}). Doing next chromosome.")
                qc_str += (
                    f'AND_set\t0\tnumber of pairs in AND set at chromosome {CHR} (no long reads file)\n'
                    f'XOR_set\t0\tnumber of pairs in XOR set at chromosome {CHR} (no long reads file)\n'
                    f'NOT_set\t0\tnumber of pairs in NOT set at chromosome {CHR} (no long reads file)\n'
                )
                continue

            ps_long = pd.read_csv(long_fname, header=None, sep='\t')
            ...
            if ps_long.shape[0]:
                ...
                ps_long['bin1_mid'] = ps_long[['read1_bin_mid', 'read2_bin_mid']].min(axis=1)
                ps_long['bin2_mid'] = ps_long[['read1_bin_mid', 'read2_bin_mid']].max(axis=1)
                ps_long['count'] = 1
                count_data = ps_long[['bin1_mid', 'bin2_mid', 'count']].groupby(['bin1_mid', 'bin2_mid']).count()
                count_data.reset_index(inplace=True)

                # Apply distance filter upfront
                count_data_filtered = count_data[(np.abs(count_data['bin1_mid'] - count_data['bin2_mid']) <= params['BIN_RANGE']) &
                                                 (np.abs(count_data['bin1_mid'] - count_data['bin2_mid']) >= 1)].copy()
                print(f"chr {CHR} unfiltered total: {count_data['count'].sum()}, filtered total: {count_data_filtered['count'].sum()}")
                ...
                if params.get('USE_MACS2', False):
                    ...
                    specialCase = False
                    if MACS2.shape[0] > 0 and MACS2.iloc[0]['end_bin'] - MACS2.iloc[0]['start_bin'] == MACS2.shape[1] - 1:
                    ...
                    count_data_and = count_data_filtered[(count_data_filtered['bin1_mid'].isin(MACS2_peak_ranges_list)) & 
                                                        (count_data_filtered['bin2_mid'].isin(MACS2_peak_ranges_list))].copy()
                    count_data_and['1D_peak_bin1'] = 1
                    count_data_and['1D_peak_bin2'] = 1

                    count_data_xor = count_data_filtered[(count_data_filtered.bin1_mid.isin(MACS2_peak_ranges_list)) ^ 
                                                        (count_data_filtered.bin2_mid.isin(MACS2_peak_ranges_list))].copy()
                    ...
                    count_data_not = count_data_filtered[~((count_data_filtered['bin1_mid'].isin(MACS2_peak_ranges_list)) | 
                                                          (count_data_filtered['bin2_mid'].isin(MACS2_peak_ranges_list)))].copy()

                    AND_sum = count_data_and['count'].sum()
                    XOR_sum = count_data_xor['count'].sum()
                    NOT_sum = count_data_not['count'].sum()
                    qc_str += (
                        f'AND_set\t{AND_sum}\tnumber of pairs in AND set at chromosome {CHR}\n'
                        f'XOR_set\t{XOR_sum}\tnumber of pairs in XOR set at chromosome {CHR}\n'
                        f'NOT_set\t{NOT_sum}\tnumber of pairs in NOT set at chromosome {CHR}\n'
                    )
                    print(f"chr {CHR} AND: {AND_sum}, XOR: {XOR_sum}, NOT: {NOT_sum}, Total: {AND_sum + XOR_sum + NOT_sum}")
                else:
                    count_data_and = count_data_filtered.copy()
                    total_sum = count_data_and['count'].sum()
                    count_data_xor = pd.DataFrame()
                    qc_str += (
                        f'AND_set\t{total_sum}\tnumber of pairs in AND set at chromosome {CHR} (no peak classification)\n'
                        f'XOR_set\t0\tnumber of pairs in XOR set at chromosome {CHR} (no peaks provided)\n'
                        f'NOT_set\t0\tnumber of pairs in NOT set at chromosome {CHR} (no peaks provided)\n'
                    )
                    print(f"chr {CHR} AND: {total_sum}")

                print('-- handling metadata\n')
                metadata = metadata_full[metadata_full['chr'] == CHR].copy()
                metadata = pd.merge(metadata, count_data_short, on=['chr', 'bin'], how='outer')
                metadata['short_count'] = metadata['short_count'].fillna(0)

                print('-- attaching genome features attributes to AND set')
                ...
                reg_and['logl'] = np.log((reg_and['effective_length1'] + 1.0) * (reg_and['effective_length2'] + 1.0) / 
                ...
                reg_and['logShortCount'] = np.log((reg_and['short_count1'] + 1.0) * (reg_and['short_count2'] + 1.0) / max_short_and)
                reg_and['bin1_mid'] = reg_and['bin1_mid'] * params['BIN_SIZE']
                reg_and['bin2_mid'] = reg_and['bin2_mid'] * params['BIN_SIZE']

                if params.get('USE_MACS2', False) and not count_data_xor.empty:
                    print('-- attaching genome features attributes to XOR set')
                    ...
                    reg_xor['logl'] = np.log((reg_xor['effective_length1'] + 1.0) * (reg_xor['effective_length2'] + 1.0) / 
                                            (params['BIN_SIZE'] * params['BIN_SIZE']))
                    ...
                    reg_xor['logShortCount'] = np.log((reg_xor['short_count1'] + 1.0) * (reg_xor['short_count2'] + 1.0) / max_short_xor)
                    reg_xor['bin1_mid'] = reg_xor['bin1_mid'] * params['BIN_SIZE']
                    reg_xor['bin2_mid'] = reg_xor['bin2_mid'] * params['BIN_SIZE']
                else:
                    reg_xor = pd.DataFrame()
            ...
            print('no short reads for chromosome ', CHR, '. Doing next chromosome')

    print('-- saving .qc.maps file\n')
    qc_fname = params['OUT_DIR'] + params['DATASET_NAME'] + '.maps.qc'
    print("final filename: " + qc_fname)
    with open(qc_fname, 'w') as qc_file:
