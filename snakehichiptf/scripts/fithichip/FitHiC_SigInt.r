...
if (is.unsorted(gene.dist) == TRUE) {
	...
	return()
    #cat(sprintf("\n **** The interaction file is unsorted --- sorting internally ****\n"))
    #interaction.data <- interaction.data[order(gene.dist), ]
    #gene.dist <- sort(gene.dist)
}
...
if (1) {
	...
	system(paste0("awk \'NR==1 {print \"#\" $0}\' ", temp_outfile, " > ", opt$OutFile))
