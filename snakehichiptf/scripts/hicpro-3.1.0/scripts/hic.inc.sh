...
get_data_type() {
    ...
    ## Return the highest possible input file type at depth 1

    nb_fq=$(find -L "$RAW_DIR" -maxdepth 1 -name "*.fastq" -o -name "*.fastq.gz" -o -name "*.fq" -o -name "*.fq.gz" | wc -l)

    nb_bam=$(find -L "$RAW_DIR" -maxdepth 1 -name "*.bam" -o -name "*.sam" | wc -l)

    nb_vpairs=$(find -L "$RAW_DIR" -maxdepth 1 -name "*.validPairs" | wc -l)

    nb_allvpairs=$(find -L "$RAW_DIR" -maxdepth 1 -name "*.allValidPairs" | wc -l)

    nb_mat=$(find -L "$RAW_DIR" -maxdepth 1 -name "*.matrix" | wc -l)
...
{
    local file="$1"
    # Use INPUT_DIR from Makefile if available, fallback to basename of resolved RAW_DIR
    echo "${INPUT_DIR:+$(basename "$INPUT_DIR")}" || echo "$(basename "$(readlink -f "$RAW_DIR")")"
}
